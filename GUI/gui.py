...
class mainWindow(object):
	...
	def __init__(self,master):
		...
		# File Menu
		options.add_command(label = "Save", command = self.save,font=('Helvetica',9,'bold'))
		options.add_command(label = "Quit", command = self.quit,font=('Helvetica',9,'bold'))
		options.add_separator()
		menuBar.add_cascade(label = "File", menu = options,font=('Helvetica',9,'bold'))
		...
		self.save_button = Tk.Button(master, text='Save', command = self.save,font=('Helvetica',9,'bold'))
		self.save_button.grid(row = 0, column = 0, sticky = Tk.W, padx = 20,
			pady = 10)
		...
		self.book_list = Tk.Listbox(master, yscrollcommand = self.scrollbar.set, 
			height=20)
		self.book_list.grid(row = 2, column = 0, rowspan = 12 , padx = 35,pady=20)
		...
		self.search_return = Tk.Button(master, text = 'Search', 
			command = self.search_call,font=('Helvetica',9,'bold'))
		self.search_return.grid(row = 0, column = 3, padx = 5,pady=20)
		...
		self.add_button = Tk.Button(master, text = 'Add', 
			command = self.popupAdd,font=('Helvetica',9,'bold'))
		self.add_button.grid(row = 14, column = 0, sticky = Tk.W , padx = 20,pady=22)

		# Delete contact button
		self.delete_button = Tk.Button(master, text = 'Delete', 
			command = self.popup_confirmation,font=('Helvetica',9,'bold'))
		self.delete_button.grid(row = 14, column = 0 )

		# Edit contact button
		self.edit_button = Tk.Button(master, text = 'Edit', 
			command = self.popupEdit,font=('Helvetica',9,'bold'))
		self.edit_button.grid(row = 14, column = 4, padx = 50, sticky = Tk.E )
		...
		self.last_name.grid(row = 3, column = 4,padx=50)
		...
		# Zip
		self.zip_label = Tk.Label(master, text= 'Pin code:')
		self.zip_label.grid(row = 8, column = 3)
		self.zip = Tk.Entry(master)
		self.zip.grid(row = 8, column = 4)

		# Home phone
		self.home_label = Tk.Label(master, text = 'TelePhone:')
		...
		# Birthday
		self.birthday_label = Tk.Label(master, text = 'DOB:')
